TokoVoipConfig = {
	...
	plugin_data = {
		TSChannel = "Jogando",
		TSPassword = "s4vb8343b41zmxc34nj12318z09skadodkamn213b4j1b487sbadbsajm32l14mk314p3124134", -- TeamSpeak channel password (can be empty)
		TSChannelWait = "Aguardando",
		TSServer = "20.197.187.171:9987", -- TeamSpeak server address to be displayed on blocking screen
		TSChannelSupport = "",
		TSDownload = "", 
		TSChannelWhitelist = { 
			"Staffs",
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = "SKZ CITY RP" -- Set the local name
		end);
